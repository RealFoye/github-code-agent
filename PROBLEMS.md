...
### 问题 21：ranker 输出 full_name 模板占位符导致匹配全部失败 ✅ 已修复

**错误现象（2026/04/12）：**
- 查询 "Python 写的、适合新手学习的 agent 项目" → 所有 22 个仓库 `relevance_score = 0.0`
- 查询 "不是框架库" → 所有 20 个仓库 `relevance_score = 0.0`
- 结果实际按 star 排序，但日志显示"等同于 fallback 效果"却没走 `_fallback_ranking()` 代码路径

**根因（Debug 日志确认）：**
- LLM 返回的 `full_name` 字段值是模板占位符 `"owner/repo"`，不是真实仓库名
- `evaluations = {}` 走的是 `JSON 解析失败` 的 `except` 分支，不是 `_fallback_ranking()` 的 `except` 分支
- `_parse_evaluation` 返回全 0 分的 cards，但外层没有检测这个条件，导致隐性降级

**修复内容（src/agent/search_agent.py）：**

**修复 1：ranker prompt 去除模板占位符**
...
并新增规则：
- full_name 必须从候选仓库列表中选取，不允许编造或使用模板占位符
- 不确定时宁可省略，不要编造

**修复 2：_parse_evaluation 增加 placeholder 检测**
...
**修复 3：rank_repos 显式 fallback 触发条件**

原逻辑：只有 `_call_llm` 抛异常才走 `_fallback_ranking()`
新逻辑：
```python
if matched_count == 0 or all_zero:
    logger.warning(f"matched_count={matched_count}, all_zero={all_zero}，显式触发 fallback")
    return self._fallback_ranking(repos, intent)
```

同时 `_parse_evaluation` 返回值从 `List[RepoCard]` 改为 `Tuple[List[RepoCard], int]`，返回 `matched_count` 供上层判断。

**回测结果（2026/04/12）：**

| 查询 | placeholder | matched_count | 显式 fallback | top1 repo |
|------|------------|--------------|--------------|----------|
| Python/新手 agent | 无 | 4 | 否 | czl9707/build-your-own-openclaw (0.850) |
| 不是框架库 | 无 | 2 | 否 | aws-samples/sample-agentic-ai-demos (0.440) |

**残留问题：**
- LLM 仍可能输出乱码/gibberish full_name（如 `mRFWq7LwNPZjaVv5v6eo/cihna-dictattorshrip-8`），不在候选列表中
- 不是 `owner/repo` 模板了，但换成了随机字符，placeholder 检测无法捕获
- 需要对乱码 full_name 做额外检测

**发生时间：** 2026/04/12

---

### 问题 22：gibberish full_name 不在候选列表中 ✅ 已修复

**错误现象（2026/04/12）：**
- 查询 "不是框架库" 时，后 3 个 repo 是乱码：`mRFWq7LwNPZjaVv5v6eo/cihna-dictattorshrip-8`
- 这些 full_name 不在候选列表中，但仍然出现在最终结果里
- placeholder 检测只能检测 `owner/repo` 模板，无法检测 LLM 瞎编的乱码

**根因：**
- 原代码先执行 `evaluations[fn] = e`（加入 evaluations dict），再检查 `fn in repo_full_names`
- gibberish full_name 不在候选列表里，但因为先加入 dict 的顺序问题或后续逻辑，没有被正确过滤

**修复内容（src/agent/search_agent.py）：**

将"先加入 dict 再检查"改为"先检查候选列表再决定是否加入"：

```python
# 修复前（有 bug）
for e in raw_evaluations:
    fn = e.get("full_name", "")
    evaluations[fn] = e  # ← 先加入
    if fn in repo_full_names:  # ← 后检查
        matched_count += 1
    else:
...
if actual_fn:
    matched_count += 1
    evaluations[actual_fn] = e  # 只有在候选列表中才加入
else:
    unmatched_full_names.append(fn)
    logger.warning(f"[gibberish_full_name] LLM 返回了不在候选列表中的 full_name: '{fn}'")
```

同时删除了原来冗余的"大小写不敏感匹配"fallback 逻辑（已在主循环中处理）。

**回测结果（2026/04/12）：**

| 查询 | matched_count | gibberish 检测 | top5 repo |
|------|-------------|--------------|----------|
| Python/新手 agent | 10 | 无 | czl9707/build-your-own-openclaw, ... |
| 不是框架库 | 5 | 无 | tinyfish-io/tinyfish-cookbook, huangjia2019/ai-agents, ... |

top5 不再出现乱码 repo，所有返回的 full_name 都在候选列表中。

**发生时间：** 2026/04/12

---

### 问题 23：教程仓库支持——结构分析失明、chunk 太粗、引用重复 ✅ 已修复
...
| 问题 | 现象 | 根因 |
|------|------|------|
| 结构分析失明 | 日志显示"0 文件, 0 行代码"，但报告很丰富 | .md 和 .ipynb 没有被识别为有效文件类型 |
| chunk 太粗 | 22 个 chunk 只能答概览 | 整个 .md 文件作为一个 chunk，没有按标题切分 |
| 引用重复 | 同一文件多次出现 | 检索结果没有去重逻辑 |
| 技术栈表不准 | 按包名录清单，不是分析式分类 | 第一阶段非目标，未处理 |

**解决方法：**

**1. code_parser.py 修改**

- FileType 枚举新增 `NOTEBOOK = "notebook"` 和 `MARKDOWN = "markdown"`
- FileClassifier.classify() 更新：
  - `.ipynb` → `NOTEBOOK`，language 默认为 "python"
  - `.md` → `MARKDOWN`
- scan_structure() 更新：NOTEBOOK 和 MARKDOWN 文件也统计行数，不再跳过

**2. chunker.py 修改**

CodeChunk 新增字段（均有默认值，向后兼容）：
```python
heading_path: str = ""                    # 语义路径，如 "Tutorial 8 > Complex Workflow"
notebook_cell_index: Optional[int] = None  # cell 序号
imports: List[str] = field(default_factory=list)  # 轻量 import 提取（正则）
```

新增 chunk_type：
- 文档类: `readme`, `tutorial_markdown`, `notebook_markdown`, `notebook_code`
- 代码类: `function`, `class`, `module`, `file`（已有）
- 其他: `config`（已有）

新增方法：
- `_chunk_markdown()`：按 `# ## ###` 标题层级切分，heading_path 存储完整路径
- `_chunk_notebook()`：解析 .ipynb JSON，按 cell 切分（markdown → notebook_markdown，code → notebook_code），用正则提取 imports

chunk_file() 路由更新：
- `.ipynb` → `_chunk_notebook()`
- `README*.md` → `_chunk_markdown(chunk_type="readme")`
- 其他 `.md` → `_chunk_markdown(chunk_type="tutorial_markdown")`

chunk_directory() 返回值更新：
```python
# 返回类型改为 Tuple[List[CodeChunk], Dict[str, Any]]
# Dict 包含: total_chunks, markdown_files, notebook_files, notebook_cells, tutorial_sections
```

**3. rag_pipeline.py 修改**

- `build_index()` 适配 `chunk_directory()` 新签名：`chunks, stats = self.chunker.chunk_directory(repo_path)`
- `_index_batch()` 元数据新增：`heading_path`, `notebook_cell_index`, `imports`
- `search()` 重构 CodeChunk 时包含新字段
- `rerank()` type_priority 更新（新增 notebook_markdown, tutorial_markdown, readme）

**4. analysis_agent.py 修改**

- `_build_context()` 显示 `heading_path`：`文件: {path} ({heading_path}) (行 {start}-{end})`
- `ask()` 去重：同一文件多个片段时保留第一个

**验收目标（2026/04/12）：**
1. ✅ 结构分析不再显示"0 文件"
2. ✅ chunk 总数从 22 增加到 50-100
3. ✅ 能回答"Tutorial 8 具体讲了什么"这类细粒度问题
4. ✅ 引用去重生效
5. ✅ 日志显示 markdown_files、notebook_files、notebook_cells、tutorial_sections、total_chunks 数量

**非目标（第一阶段明确不做）：**
- 不改 search_agent.py
- 不引入 tree-sitter notebook 深解析
- 不实现 fenced_code
- 不重做技术栈分类逻辑
- 不新增 Mermaid 架构图
- 不处理问题 7/8/9/11/12
...
### 问题 24：Tutorial 编号检索被总结文件短路 ✅ 已修复

**问题描述：**
- 问 "Tutorial 8 具体讲了什么？" 时，系统回答"无法回答"
- 引用来源全是 Tutorial 1、10、12、13 的 README
- **没有命中 Tutorial08 相关 chunks**

**根因分析：**
1. 检索阶段使用的向量 embedding 是纯语义匹配
2. 中文问题 "Tutorial 8 具体讲了什么？" 与英文 Tutorial08 文件的向量相似度不够高
3. 向量检索召回的 top_k 个 chunks 中没有 Tutorial08 相关内容
4. 后续 rerank 和去重也无法弥补检索阶段的缺失

**解决方案：**

在 `rag_pipeline.py` 中新增检索增强：

1. **`detect_tutorial_number(query)` 函数**
   - 识别中文 pattern：`第8章`
   - 识别英文 pattern：`Tutorial 8`、`tutorial8`、`tutorial_8`、`Tutorial08`

2. **`search()` 增强检索**
   - 接收 `tutorial_hint` 参数
   - 检测到 tutorial 编号时，追加英文关键词：`tutorial{tutorial_hint} tutorial_{tutorial_hint} tutorial {tutorial_hint}`
   - 改善跨语言检索（中文问句 → 英文内容）

3. **`rerank()` 优先排序**
   - 接收 `tutorial_hint` 参数
   - 对 `file_path` 或 `heading_path` 包含匹配 pattern 的 chunks 加权 +10
   - 优先将对应 tutorial 的 chunks 排到前面

4. **`analysis_agent.ask()`**
   - 先调用 `detect_tutorial_number()` 检测编号
   - 将 `tutorial_hint` 传给 `search()` 和 `rerank()`

**回测结果（2026/04/12）：**

| 查询 | 检测到 | 回答 | Top 引用 |
|------|--------|------|---------|
| Tutorial 8 具体讲了什么？ | 8 | ✅ 能回答 | Tutorial08/README.md 在 top 2 |

**验收：**
- ✅ "Tutorial 8 具体讲了什么？" 回答 Tutorial08 内容
- ✅ 引用来源包含 Tutorial08/README.md

**发生时间：** 2026/04/12

---

### 问题 25：项目概览类问题被辅助文档短路 ✅ 已修复

**问题描述：**
- 问"这个项目是干什么的？"时，回答引用了 CONTRIBUTING、CODE_OF_CONDUCT、DEVELOPMENT_GUIDE
- README 没有被召回作为主要引用来源
- 系统回答质量明显受影响
...
1. **检索阶段**：向量检索已能召回 README（score=0.671，排名第1）

2. **rerank 阶段**：路由策略没有生效
   - `detect_query_intent()` 成功识别 "project_overview"
   - 但 priority pattern 大小写不匹配：`r'(^|/)README'`（大写）vs `fp_lower`（小写）
   - 导致 README 永远匹配不到 priority pattern
   - 辅助文档 CONTRIBUTING/DEVELOPMENT_GUIDE 反而排名靠前

3. **最终结果**：回答引用 CONTRIBUTING 等文档，而非 README

**解决方案：**

1. **rag_pipeline.py 新增 `detect_query_intent(query)` 函数**
   - 识别"项目概览类问题"：这个项目是干什么的、有什么功能、项目简介等
   - 返回 intent 类型：`project_overview` / `tutorial_detail` / `general`

2. **rag_pipeline.py `rerank()` 新增 `query_intent` 参数**
   - 对 `project_overview` 意图应用源文件路由策略：
     - 优先（effective_type_priority = 1）：`(^|/)readme`、docs/index、docs/overview 等
     - 降权（+20）：contributing、code_of_conduct、license 等
   - **Bug 修复**：priority pattern 从大写 `README` 改为小写 `readme`，与 `fp_lower` 匹配

3. **analysis_agent.py `ask()` 更新**
   - Step 1：同时检测 `query_intent` 和 `tutorial_hint`
   - Step 2：传给 `search()` 和 `rerank()`

**pattern 大小写 bug 详情：**
```python
# 错误：pattern 大写，路径小写，永远不匹配
priority_patterns = [r'(^|/)README']  # 大写
fp_lower = '/a/b/readme_cn.md'        # 小写

# 正确：统一小写
priority_patterns = [r'(^|/)readme']   # 小写
fp_lower = '/a/b/readme_cn.md'        # 小写
```

**回测结果（2026/04/12）：**

| 仓库 | 查询 | 检测到 intent | Top 引用 |
|------|------|--------------|---------|
| Mini-Agent | 这个项目是干什么的？ | project_overview | #1 README_CN.md ✅ |

**验收：**
- ✅ README 出现在 top 3 引用来源
- ✅ 回答内容正确（识别为 MiniMax AI Mini Agent 项目）
- ✅ CONTRIBUTING/CODE_OF_CONDUCT 被降权
