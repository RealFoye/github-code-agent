...
**残留问题：**
`excluded_tech` 硬过滤已生效（Q5 LangChain 排除验证通过），但 `excluded_categories`（否定类型）软降权**仍然不可靠**：
- 查询 Q2："不是框架库" → `parallel75/AI_Agent`（描述含 LangChain Tutorial）未被 penalty，仍排第 2
- 查询 Q2（本次测试）：政治内容仓库 `cirosantilli/china-dictatorship`（score=0.00）仍出现在 top 5，说明 LLM 没有把"不是框架库"正确映射为 `excluded_categories: ["framework", "library"]`，且软 penalty 力度不足以将无关仓库挤出前 5
- **根因**：LLM 意图解析时对"不是 X"的类型否定识别率低，且即使识别了，`excluded_categories` 也只是 ranking prompt 中的软指导，不如硬过滤可靠

**发生时间：** 2026/04/12

---

### 问题 20：JSON 解析失败率高，导致大量 fallback 到 star-based ranking

**错误现象：**
- 查询 Q1："Python 写的、适合新手学习的 agent 项目" → JSON 解析失败，错误 `Expecting ',' delimiter: line 8 column 28 (char 190)`，fallback 到 star-based，结果全是 matplotlib/python 基础教程
- 查询 Q4（历史）：JSON 解析失败，`Expecting ',' delimiter`，fallback 到 star-based
- 离线模拟测试：Q2 风格（`suitability_reasons` 含中文引号/括号，数组截断）和 Q4 风格（尾随多余文字）是最常见的坏 JSON 模式
...
| Q2: 不是框架库 | ❌ 否 | ✅ 是 | ✅ 是（JSON 解析失败后 fallback 重建 27 张卡，score 全 0） |
| Q3: 小而美 | ❌ 否 | ✅ 是 | ❌ 否 |

**残留问题：**
- `_cleanup_json_string` 在本次 3 个查询中**未被触发**（因为 JSON 解析本身就成功了）
- Q2 fallback 的根因是 LLM 评估阶段的 529 overload 错误，而非 JSON 格式问题
- cleanup 逻辑在本次测试中**未观察到实际救回**的真实 case
- 最常见的坏 JSON 模式（中文引号/括号截断）理论上能被 full_name_scan 救回，但缺乏 live 验证

**下一步改进方向：**
若 cleanup 仍未被触发，考虑在意图解析阶段（`_llm_parse`）也加入 JSON 修复逻辑，或者增加 prompt 约束让 LLM 避免在 suitability_reasons 中使用引号类字符。
